...
function App() {
  ...
  function openNaverMap(region) {
    window.open(`https://map.naver.com/p/search/${encodeURIComponent(region.ko)}`, "_blank");
  }
    ...
    <main className={`app ${isSenior ? "senior-mode" : ""}`}>
      ...
      <section className="dashboard-layout">
        ...
        <section className="map-card">
          <div className="map-badge">🗺️ {isEnglish ? "Weighted Recommendation Map" : "가중치 기반 추천 지도"}</div>
        ...
        <aside className="right-panel">
          <div className="results-head">
            ...
            <p>{isEnglish ? "Calculated from the five category weights." : "5대 카테고리 가중치를 기반으로 계산되었습니다."}</p>
          </div>
          ...
          <div className="result-list">
            {rankings.map((region, index) => (
              <RecommendationCard
                ...
                onDetail={() => setDetailRegion(region)}
              />
            ))}
          </div>
        </aside>
      ...
      <section className={`detail-modal ${isSenior ? "senior-modal" : ""}`}>
        ...
        <div className="detail-body">
          <aside className="detail-left">
            <div className="region-image-card"><span>{isEnglish ? region.en : region.short}</span></div>
          ...
          <main className="detail-right">
            ...
            {mode === "detail" && (
              <table className="detail-table">
                ...
                <tbody>
                  {(isEnglish ? Object.values(INDICATOR_EN_SECTIONS).flatMap((section) => section.rows) : INDICATOR_SECTIONS.flatMap((section) => section.rows)).slice(0, 5).map((row, index) => (
                    <tr key={`${row[0]}-${index}`}><td>{row[0]}</td><td>{Math.round(25 + index * 9)}</td><td>{row[1]}</td><td>{region.scores[CATEGORIES[index % CATEGORIES.length].key]}</td><td>{row[4]}</td></tr>
                  ))}
                </tbody>
              </table>
            )}
