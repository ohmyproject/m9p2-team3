...
const GeoJsonThreeMap = lazy(() => import("./components/GeoJsonThreeMap.jsx").then((module) => ({ default: module.GeoJsonThreeMap })));
...
class MapErrorBoundary extends React.Component {
  ...
  static getDerivedStateFromError(error) {
    return {
      hasError: true,
      message: error?.message || "3D map rendering error"
    };
  }

  componentDidCatch(error) {
    console.error("3D 지도 렌더링 오류:", error);
  }

  render() {
    if (this.state.hasError) {
      return <MapFallback isEnglish={this.props.isEnglish} message={this.state.message} />;
    }
    return this.props.children;
  }
}

function MapFallback({ isEnglish, message }) {
  return (
    <div className="geojson-map-state map-fallback-state">
      <strong>{isEnglish ? "3D map is not ready." : "3D 지도를 표시하지 못했습니다."}</strong>
      <span>
        {isEnglish
          ? "Run npm install in the frontend folder, then restart npm run dev."
          : "frontend 폴더에서 npm install을 실행한 뒤 npm run dev를 다시 실행해 주세요."}
      </span>
...
const REGIONS = [
  {
    id: "gangwon", ko: "강원특별자치도", en: "Gangwon", short: "강원",
  ...
  {
    id: "gyeonggi", ko: "경기도", en: "Gyeonggi", short: "경기",
  ...
  {
    id: "chungbuk", ko: "충청북도", en: "Chungbuk", short: "충북",
  ...
  {
    id: "jeonnam", ko: "전라남도", en: "Jeonnam", short: "전남",
  ...
  {
    id: "gyeongbuk", ko: "경상북도", en: "Gyeongbuk", short: "경북",
  ...
  {
    id: "jeju", ko: "제주특별자치도", en: "Jeju", short: "제주",
  ...
  {
    id: "seoul", ko: "서울특별시", en: "Seoul", short: "서울",
  ...
  {
    id: "busan", ko: "부산광역시", en: "Busan", short: "부산",
...
const INDICATOR_SECTIONS = [
  {
    key: "traffic", title: "교통 (Traffic)", icon: "▣", variant: "green",
    rows: [
  ...
  {
    key: "culture", title: "문화 (Culture)", icon: "◇", variant: "plum",
    rows: [
  ...
  {
    key: "convenience", title: "생활편의 (Convenience)", icon: "▥", variant: "light",
    rows: [
...
function normalizeApiRegion(item, weights) {
  ...
  const region = {
    ...
    ko, en,
    short: item.shortName || item.short_name || ko, // 💡 잘리는 문제 방지를 위해 ko 로 폴백
    ...
    geoCode: item.geo_code || item.geoCode || item.sig_cd || item.SIG_CD || item.region_code || "",
...
function App() {
  ...
  const [mapZoom, setMapZoom] = useState(0.95);
  ...
  function changeMapZoom(delta) {
    setMapZoom((zoom) => {
      const nextZoom = Math.max(0.4, Math.min(8.0, Number((zoom + delta).toFixed(2))));
      return nextZoom;
    });
  }
    ...
    <main className={`app ${isSenior ? "senior-mode" : ""}`}>
      ...
      <section className="dashboard-layout">
        ...
        <section className="map-card">
          ...
          >
            <MapErrorBoundary isEnglish={isEnglish}>
              <Suspense fallback={<MapFallback isEnglish={isEnglish} />}>
                <GeoJsonThreeMap
                  rankings={rankings}
...
function RecommendationCard({ region, rank, selected, isEnglish, isSenior, isDetail, onSelect, onDetail, onOpenMap }) {
    ...
    <article className={`result-card ${selected ? "selected" : ""}`} onClick={onSelect}>
      ...
      <div className="result-main">
        ...
        <div className="result-text">
          ...
          <p className="region-sub">{region.en}</p>
      ...
      <div className="result-bottom">
        ...
        <button onClick={(event) => { event.stopPropagation(); isDetail || isSenior ? onDetail() : onOpenMap?.(); }}>{isDetail || isSenior ? (isEnglish ? "View Details" : "상세 보기") : (isEnglish ? "Naver Map" : "네이버 지도")}</button>
      </div>
      {isDetail && (
    ...
    <div className="modal-backdrop">
      <section className={`detail-modal ${isSenior ? "senior-modal" : ""}`}>
        ...
        <header className="detail-header">
          ...
          {/* 💡 글자 잘림을 방지하기 위해 전체 이름(region.ko) 출력 */}
          <h1>{isEnglish ? region.en : region.ko}</h1>
