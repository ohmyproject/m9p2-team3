import React, { useEffect, useMemo, useState } from "react";import "./App.css";
...
function getRegionId(region) {
  return region?.region_id || region?.id || region?.regionId;
}

function normalizeWeights(weights) {
  return {
    traffic: Number(weights?.traffic ?? 20),
    culture: Number(weights?.culture ?? 20),
    convenience: Number(weights?.convenience ?? 25),
    safety: Number(weights?.safety ?? 20),
    nature: Number(weights?.nature ?? 15),
  };
}
  ...
  React.useEffect(() => {
    ...
    async function fetchRagInsight() {
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/ai/explain-region`, {
          method: "POST",
          headers: {
      ...
      <div className="rag-insight-header">
        <div className="rag-title-row">
          <span className="rag-badge">
            {isFallback ? "Fallback" : "RAG"}
          </span>
          <span className="rag-sub-badge">
            {isEnglish ? "AI Generated Insight" : "AI 추천 근거"}
          </span>
        </div>
      ...
      {ragLoading && (
        <div className="rag-loading">
          {isEnglish
            ...
            : "AI가 추천 근거를 생성하는 중입니다..."}
        </div>
      )}

      {ragError && !ragLoading && (
        <div className="rag-error">{ragError}</div>
      )}
